...
describe("typereference", function() {
  ...
  it("callable (argument)", function() {
    expect(parser.parseEval('function foo(callable $arg) {}')).toMatchSnapshot();
  });
  it("callable (argument) (uppercase)", function() {
    expect(parser.parseEval('function foo(CALLABLE $arg) {}')).toMatchSnapshot();
  });
  it("callable (return type declarations)", function() {
    expect(parser.parseEval('function foo($arg): callable {}')).toMatchSnapshot();
  });
  ...
  it("iterable (argument)", function() {
    expect(parser.parseEval('function foo(iterable $arg) {}')).toMatchSnapshot();
  });
  it("iterable (argument) (uppercase)", function() {
    expect(parser.parseEval('function foo(ITERABLE $arg) {}')).toMatchSnapshot();
  });
  it("iterable (return type declarations)", function() {
    expect(parser.parseEval('function foo($arg): iterable {}')).toMatchSnapshot();
  });
  ...
  it("class (4)", function() {
    expect(parser.parseEval('function foo($arg): Foo\\Foo {}')).toMatchSnapshot();
  });
});
