...
describe("Test strings", function() {
  ...
  it("fix #124", function() {
    expect(parser.parseEval('$string = "He drank some $juices[koolaid1] juice.";')).toMatchSnapshot();
  });

  it("fix #123", function() {
    expect(parser.parseEval("$string = 'Avoid converting \n chars, but \\' or \\\\ is ok.';")).toMatchSnapshot();
  });
  ...
  it("test binary with double quotes", function() {
    expect(parser.parseEval(`echo b"\\colors contains >$colors<\\n";`)).toMatchSnapshot();
  });

  describe("check infinite on $", function() {
    it("using doublequotes", function() {
      expect(parser.parseEval(`echo "$`, {
  ...
  it.skip("binary cast", function() {
    expect(parser.parseEval(`echo (binary)"\\colors[1] contains >$colors[1]<\\n";`)).toMatchSnapshot();
  });
  ...
  it("test nowdoc label and contents", function() {
    expect(parser.parseEval(["$code .= <<<'EOF'", "  }", "EOF;"].join("\r\n"), {
      parser: {
        debug: false
      }
    })).toMatchSnapshot();
  ...
  it("test heredoc end of doc", function() {
    expect(parser.parseEval(["$a = <<<EOF2", "SOMETHING"].join("\r"), {
      parser: {
        suppressErrors: true
      }
    })).toMatchSnapshot();
  ...
  it("test nowdoc end of doc", function() {
    expect(parser.parseEval(["$a = <<<'EOF2'", "FOO"].join("\r"), {
      parser: {
        suppressErrors: true
      }
    })).toMatchSnapshot();
